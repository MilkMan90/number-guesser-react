    ...
    return(
      ...
      <button className = 'new-game-button' onClick={()=>this.props.newGame}>Reset Game</button>
        <div className = 'min-max-wrapper'>
          ...
          <input type='number' value ={this.state.min} className = 'min-input-field' placeholder={this.state.min} onChange={(e)=> this.setState({min: e.target.value})}/>
          <input type='number' value ={this.state.max} className = 'max-input-field' placeholder={this.state.max} onChange={(e)=> this.setState({max: e.target.value})}/>
          <button className = 'set-range-button' onClick={()=>this.props.handleNewRange(this.state.min, this.state.max )}>Set New Range</button>
        </div>
      </div>
    )
  }
